...
async function install_luajit(luaInstallPath, luaVersion) {
  ...
  await exec.exec(
    `git clone --branch ${repo.branch} --single-branch ${repo.url}`,
    undefined,
    {
      cwd: buildPath,
    },
  ...
  await finish_luajit_install(
    pathJoin(buildPath, baseDir, "src"),
    luaInstallPath,
    repo.binary,
  );
}
...
const makeCacheKey = (luaVersion, compileFlags) =>
  `lua-${luaVersion}-${process.platform}-${process.arch}${compileFlags ? `-${compileFlags}` : ""}`;
